...
Before continuing make sure the Lumen site is available at <http://127.0.0.1> and that the jouled service is running:


```{.bash title="SHELL"}
# ensure the joule client is installed
$> joule --version
joule, version 1.X.X

# ensure the joule server (jouled) is running
$> sudo systemctl status joule.service
```

Assuming `jouled` is running on the local system (not in a docker container), use the following command to authorize your client. This requires admin privileges, subsequent users can be added without admin privileges using the [CLI](cli.md) ``add user`` command which can be executed from a remote system (a different computer from the server running `jouled`). 
...
![Getting Started Pipeline](images/getting_started_pipeline.png)
...
![Quick Start Plot](images/quickstart-plot.png)
...
![Quick Start Data App](images/data-app.png)
